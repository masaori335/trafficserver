...
class CompressPluginTest:
    ...
    def setupTS(self):
        ...
        self.ts.Setup.Copy("etc/cache-false-ignore-range.config")
        self.ts.Setup.Copy("etc/cache-false-no-compression.config")
        self.ts.Setup.Copy("etc/cache-true-ignore-range.config")
        self.ts.Setup.Copy("etc/cache-true-no-compression.config")
        self.ts.Disk.remap_config.AddLines(
            {
                f"map /cache-false-ignore-range/   http://127.0.0.1:{self.server.Variables.http_port}/ @plugin=compress.so @pparam={Test.RunDirectory}/cache-false-ignore-range.config",
                f"map /cache-false-no-compression/ http://127.0.0.1:{self.server.Variables.http_port}/ @plugin=compress.so @pparam={Test.RunDirectory}/cache-false-no-compression.config",
