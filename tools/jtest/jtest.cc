...
poll_init(int sock)
{
  if (!fd[sock].req_header) {
    size_t buf_size              = HEADER_SIZE * pipeline + MAX_REQUEST_BODY_LENGTH;
    fd[sock].req_header_buf_size = buf_size;
    fd[sock].req_header          = (char *)malloc(buf_size);
  }
...
make_response(int sock, int code)
{
  ...
  fd[sock].length          = snprintf(fd[sock].req_header, fd[sock].req_header_buf_size, "%d\r\n", code);
  fd[sock].req_pos         = 0;
...
#define STREQ(_x, _s) (!strncasecmp(_x, _s, sizeof(_s) - 1))
    ...
    Lhere:
      ...
      fd[sock].length = snprintf(fd[sock].req_header, fd[sock].req_header_buf_size, "213 %d\r\n", atoi(buffer + 5));
      ...
      if (err_rand < ftp_mdtm_err_rate) {
        fd[sock].length = snprintf(fd[sock].req_header, fd[sock].req_header_buf_size, "550 mdtm file not found\r\n");
      } else {
        if (ftp_mdtm_rate == 0) {
          fd[sock].length = snprintf(fd[sock].req_header, fd[sock].req_header_buf_size, "213 19900615100045\r\n");
        } else {
          time_t mdtm_now;
          time(&mdtm_now);
          ...
          fd[sock].length = snprintf(fd[sock].req_header, fd[sock].req_header_buf_size, "%s\r\n", ftp_mdtm_str);
        }
      }
      make_long_response(sock);
      return 0;
      ...
      fd[sock].length =
        snprintf(fd[sock].req_header, fd[sock].req_header_buf_size, "227 (%u,%u,%u,%u,%u,%u)\r\n",
      ...
      fd[sock].length                                 = snprintf(fd[sock].req_header, fd[sock].req_header_buf_size, "200 Okay\r\n");
      if (verbose) {
        puts(fd[sock].req_header);
      }
      make_long_response(sock);
      ...
      fd[sock].response = fd[sock].req_header;
      fd[sock].length =
        snprintf(fd[sock].req_header, fd[sock].req_header_buf_size, "150 %d bytes\r\n", fd[fd[sock].ftp_data_fd].length);
      fd[sock].req_pos         = 0;
      fd[sock].response_length = strlen(fd[sock].req_header);
      poll_set(sock, nullptr, write_ftp_response);
      return 0;
                    ...
                    const char *cookie)
{
  ...
  case 0:
    if (range_mode) {
      snprintf(fd[sock].req_header, fd[sock].req_header_buf_size,
...
build_request(int sock)
{
  ...
  *eheaders    = 0;
  ...
  if (nheaders > 0) {
    char *eh        = eheaders;
    const char *end = eh + sizeof(eheaders);
    if (!vary_user_agent) {
      eh += snprintf(eh, end - eh, "User-Agent: Mozilla/4.04 [en] (X11; I; Linux 2.0.31 i586)\r\n");
      nheaders--;
    }
    if (nheaders > 0) {
      eh += snprintf(eh, end - eh, "Accept: image/gif, image/x-xbitmap, image/jpeg, image/pjpeg, image/png, */*\r\n");
    }
    while (--nheaders > 0) {
      eh += snprintf(eh, end - eh, "Extra-Header%d: a lot of junk for header %d\r\n", nheaders, nheaders);
    }
  }
  char cookie[256];
  *cookie             = 0;
