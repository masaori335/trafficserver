      ...
      if (!e->flag_bits.lru) { // in memory
        ...
        if (e->flag_bits.compressed) {
          ...
          if (!e->flag_bits.copy) { // don't bother if we have to copy anyway
            ...
            this->_bytes  += delta;
            Metrics::increment(cache_rsb.ram_cache_bytes, delta);
            Metrics::increment(stripe->cache_vol->vol_rsb.ram_cache_bytes, delta);
        ...
        Metrics::increment(stripe->cache_vol->vol_rsb.ram_cache_hits);
        ...
        Metrics::increment(stripe->cache_vol->vol_rsb.ram_cache_misses);
...
RamCacheCLFUS::_destroy(RamCacheCLFUSEntry *e)
{
  ...
  if (!e->flag_bits.lru) {
    ...
    Metrics::decrement(cache_rsb.ram_cache_bytes, e->size);
    Metrics::decrement(stripe->cache_vol->vol_rsb.ram_cache_bytes, e->size);
    e->data = nullptr;
  } else {
    this->_history--;
  }
...
RamCacheCLFUS::_requeue_victims(Que(RamCacheCLFUSEntry, lru_link) & victims)
{
  ...
  while ((victim = victims.dequeue())) {
    ...
    Metrics::increment(cache_rsb.ram_cache_bytes, victim->size);
    Metrics::increment(stripe->cache_vol->vol_rsb.ram_cache_bytes, victim->size);
...
RamCacheCLFUS::put(CryptoHash *key, IOBufferData *data, uint32_t len, bool copy, uint64_t auxkey)
{
  ...
  while (true) {
    ...
    this->_bytes -= victim->size + ENTRY_OVERHEAD;
    Metrics::decrement(cache_rsb.ram_cache_bytes, victim->size);
    Metrics::decrement(stripe->cache_vol->vol_rsb.ram_cache_bytes, victim->size);
...
Linsert:
  ...
  Metrics::increment(cache_rsb.ram_cache_bytes, size);
  Metrics::increment(stripe->cache_vol->vol_rsb.ram_cache_bytes, size);
  e->size = size;
  this->_objects++;
