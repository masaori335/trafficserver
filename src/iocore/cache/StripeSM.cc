...
StripeSM::clear_dir()
{
  stripe_write_op([&](Stripe *stripe) { stripe->clear_init(); });

  return stripe_read_op<int>([&](const Stripe *stripe) {
    if (pwrite(this->fd, stripe->raw_dir, stripe->dirlen(), stripe->skip) < 0) {
...
StripeSM::init(char *s, off_t blocks, off_t dir_skip, bool clear)
{
  ...
  return this->stripe_read_op<int>([&](const Stripe *stripe) {
    this->hit_evacuate_window = (stripe->data_blocks * cache_config_hit_evacuate_percent) / 100;

    // PreservationTable
    this->_preserved_dirs.evacuate_size = static_cast<int>(stripe->len / EVACUATION_BUCKET_SIZE) + 2;
    ...
    Dbg(dbg_ctl_cache_init, "Vol %s: allocating %zu directory bytes for a %lld byte volume (%lf%%)", hash_text.get(),
        stripe->dirlen(), static_cast<long long>(stripe->len),
        static_cast<double>(stripe->dirlen()) / static_cast<double>(stripe->len) * 100.0);
    ...
    off_t footer_offset = stripe->dirlen() - footerlen;
    // try A
    off_t as = stripe->skip;
    ...
    off_t bs                               = stripe->skip + stripe->dirlen();
...
{
  int res = this->stripe_read_op<int>([&](const Stripe *stripe) {
    ...
    if (event == AIO_EVENT_DONE) {
      ...
      if (op->aiocb.aio_nbytes == dir_len) {
        ...
        op->aiocb.aio_offset = stripe->skip + dir_len;
...
StripeSM::handle_dir_read(int event, void *data)
{
  ...
  int res = this->stripe_write_op<int>([&](Stripe *stripe) {
    if (!(stripe->header->magic == STRIPE_MAGIC && stripe->footer->magic == STRIPE_MAGIC &&
          CACHE_DB_MAJOR_VERSION_COMPATIBLE <= stripe->header->version._major &&
      ...
      Note("STRIPE_MAGIC %d\n header magic: %d\n footer_magic %d\n CACHE_DB_MAJOR_VERSION_COMPATIBLE %d\n major version %d\n"
           ...
           STRIPE_MAGIC, stripe->header->magic, stripe->footer->magic, CACHE_DB_MAJOR_VERSION_COMPATIBLE,
           stripe->header->version._major, CACHE_DB_MAJOR_VERSION);
      Note("clearing cache directory '%s'", hash_text.get());
      clear_dir_aio();
      return EVENT_DONE;
    ...
    stripe->sector_size = stripe->header->sector_size;
    return EVENT_CONT;
  });
...
StripeSM::clear_dir_aio()
{
  this->stripe_write_op([&](Stripe *stripe) {
    ...
    io.aiocb.aio_buf    = stripe->raw_dir;
    io.aiocb.aio_nbytes = stripe->dirlen();
    io.aiocb.aio_offset = stripe->skip;
    io.action           = this;
...
StripeSM::handle_recover_from_data(int event, void * /* data ATS_UNUSED */)
{
  return this->stripe_read_op<int>([&](const Stripe *stripe) {
    uint32_t got_len         = 0;
    uint32_t max_sync_serial = stripe->header->sync_serial;
    char    *s, *e = nullptr;
    if (event == EVENT_IMMEDIATE) {
      if (stripe->header->sync_serial == 0) {
      ...
      last_sync_serial  = 0;
      last_write_serial = 0;
      recover_pos       = stripe->header->last_write_pos;
      if (recover_pos >= stripe->skip + stripe->len) {
        recover_wrapped = true;
        recover_pos     = stripe->start;
      }
      io.aiocb.aio_buf    = static_cast<char *>(ats_memalign(ats_pagesize(), RECOVERY_SIZE));
      io.aiocb.aio_nbytes = RECOVERY_SIZE;
      if (static_cast<off_t>(recover_pos + io.aiocb.aio_nbytes) > static_cast<off_t>(stripe->skip + stripe->len)) {
        io.aiocb.aio_nbytes = (stripe->skip + stripe->len) - recover_pos;
      }
      ...
      if (io.aiocb.aio_offset == stripe->header->last_write_pos) {
        ...
        */
        uint32_t to_check = stripe->header->write_pos - stripe->header->last_write_pos;
        ...
        while (done < to_check) {
          ...
          if (doc->magic != DOC_MAGIC || doc->write_serial > stripe->header->write_serial) {
            Warning("no valid directory found while recovering '%s', clearing", hash_text.get());
            goto Lclear;
          }
          done += stripe->round_to_approx_size(doc->len);
    ...
    if (got_len) {
      ...
      if (recover_wrapped && stripe->start == io.aiocb.aio_offset) {
        doc = reinterpret_cast<Doc *>(s);
        if (doc->magic != DOC_MAGIC || doc->write_serial < last_write_serial) {
          recover_pos = stripe->skip + stripe->len - EVACUATION_SIZE;
          goto Ldone;
        }
      }
      ...
      while (s < e) {
        ...
        if (doc->magic != DOC_MAGIC || doc->sync_serial != last_sync_serial) {
          if (doc->magic == DOC_MAGIC) {
            if (doc->sync_serial > stripe->header->sync_serial) {
            ...
            */
            ...
            // case 1
            // case 2
            if (doc->sync_serial > last_sync_serial && doc->sync_serial <= stripe->header->sync_serial + 1) {
              last_sync_serial  = doc->sync_serial;
              s                += stripe->round_to_approx_size(doc->len);
              continue;
            }
            ...
            else if (recover_pos - (e - s) > (stripe->skip + stripe->len) - AGG_SIZE) {
              recover_wrapped     = true;
              recover_pos         = stripe->start;
              io.aiocb.aio_nbytes = RECOVERY_SIZE;
            ...
            recover_pos -= e - s;
            if (recover_pos > (stripe->skip + stripe->len) - AGG_SIZE) {
              recover_wrapped     = true;
              recover_pos         = stripe->start;
              io.aiocb.aio_nbytes = RECOVERY_SIZE;
  ...
  Ldone: {
    ...
    if (recover_pos == stripe->header->write_pos && recover_wrapped) {
      SET_HANDLER(&StripeSM::handle_recover_write_dir);
    ...
    if (recover_pos < stripe->header->write_pos && (recover_pos + EVACUATION_SIZE >= stripe->header->write_pos)) {
      Dbg(dbg_ctl_cache_init, "Head Pos: %" PRIu64 ", Rec Pos: %" PRIu64 ", Wrapped:%d", stripe->header->write_pos, recover_pos,
          recover_wrapped);
      Warning("no valid directory found while recovering '%s', clearing", hash_text.get());
      goto Lclear;
    }
    ...
    if (recover_pos > stripe->skip + stripe->len) {
      recover_pos -= stripe->skip + stripe->len;
    }
    ...
    if (!(stripe->header->sync_serial & 1) == !(next_sync_serial & 1)) {
      next_sync_serial++;
    }
    // clear effected portion of the cache
    off_t clear_start = stripe->offset_to_vol_offset(stripe->header->write_pos);
    off_t clear_end   = stripe->offset_to_vol_offset(recover_pos);

    this->stripe_write_op([&](Stripe *stripe) {
      if (clear_start <= clear_end) {
        dir_clear_range(clear_start, clear_end, stripe);
      } else {
        dir_clear_range(clear_start, DIR_OFFSET_MAX, stripe);
        dir_clear_range(1, clear_end, stripe);
      }
    });

    Note("recovery clearing offsets of Stripe %s : [%" PRIu64 ", %" PRIu64 "] sync_serial %d next %d\n", hash_text.get(),
         stripe->header->write_pos, recover_pos, stripe->header->sync_serial, next_sync_serial);

    stripe->footer->sync_serial = stripe->header->sync_serial = next_sync_serial;
    ...
    int    B         = stripe->header->sync_serial & 1;
    off_t  ss        = stripe->skip + (B ? dirlen : 0);

    init_info->vol_aio[0].aiocb.aio_buf    = stripe->raw_dir;
    init_info->vol_aio[0].aiocb.aio_nbytes = footerlen;
    init_info->vol_aio[0].aiocb.aio_offset = ss;
    init_info->vol_aio[1].aiocb.aio_buf    = stripe->raw_dir + footerlen;
    init_info->vol_aio[1].aiocb.aio_nbytes = dirlen - 2 * footerlen;
    init_info->vol_aio[1].aiocb.aio_offset = ss + footerlen;
    init_info->vol_aio[2].aiocb.aio_buf    = stripe->raw_dir + dirlen - footerlen;
...
StripeSM::handle_recover_write_dir(int /* event ATS_UNUSED */, void * /* data ATS_UNUSED */)
{
  ...
  this->stripe_write_op([&](Stripe *stripe) {
    stripe->scan_pos = stripe->header->write_pos;
    ink_assert(this->mutex->thread_holding == this_ethread());
    this->_preserved_dirs.periodic_scan(stripe);
  });
  ...
  case AIO_EVENT_DONE:
    ...
    return this->stripe_read_op<int>([&](const Stripe *stripe) {
      ...
      // try B
        ...
        io.aiocb.aio_offset = stripe->skip + stripe->dirlen();
        ink_assert(ink_aio_read(&io));
      } else {
        Note("no good directory, clearing '%s' since sync_serials on both A and B copies are invalid", hash_text.get());
        Note("Header A: %d\nFooter A: %d\n Header B: %d\n Footer B %d\n", hf[0]->sync_serial, hf[1]->sync_serial,
             hf[2]->sync_serial, hf[3]->sync_serial);
        clear_dir_aio();
        delete init_info;
        init_info = nullptr;
      }
...
StripeSM::aggWriteDone(int event, Event *e)
{
  ...
  if (io.ok()) {
    this->stripe_write_op([&](Stripe *stripe) {
      ...
      stripe->header->write_pos      += io.aiocb.aio_nbytes;
      ink_assert(stripe->header->write_pos >= stripe->start);
      DDbg(dbg_ctl_cache_agg, "Dir %s, Write: %" PRIu64 ", last Write: %" PRIu64 "", hash_text.get(), stripe->header->write_pos,
           stripe->header->last_write_pos);
      ink_assert(stripe->header->write_pos == stripe->header->agg_pos);
      if (stripe->header->write_pos + EVACUATION_SIZE > stripe->scan_pos) {
        ink_assert(this->mutex->thread_holding == this_ethread());
        this->stripe_write_op([&](Stripe *stripe) { this->_preserved_dirs.periodic_scan(stripe); });
      }
    ...
    this->stripe_read_op([&](const Stripe *stripe) {
      for (int done = 0; done < this->_write_buffer.get_buffer_pos();) {
        ...
        dir_set_offset(&del_dir, stripe->header->write_pos + done);
...
StripeSM::aggWrite(int event, void * /* e ATS_UNUSED */)
{
  ...
  return this->stripe_write_op<int>([&](Stripe *stripe) {
    Lagain:
      ...
      // evacuate space
      off_t end = stripe->header->write_pos + this->_write_buffer.get_buffer_pos() + EVACUATION_SIZE;
      if (evac_range(stripe->header->write_pos, end, !stripe->header->phase) < 0) {
        goto Lwait;
      }
      if (end > stripe->skip + stripe->len) {
        if (evac_range(stripe->start, stripe->start + (end - (stripe->skip + stripe->len)), stripe->header->phase) < 0) {
          goto Lwait;
        }
      }
      ...
      if (this->_write_buffer.is_empty()) {
        ...
        int l = stripe->round_to_approx_size(sizeof(Doc));
        this->_write_buffer.seek(l);
        Doc *d = reinterpret_cast<Doc *>(this->_write_buffer.get_buffer());
        memset(static_cast<void *>(d), 0, sizeof(Doc));
        d->magic        = DOC_MAGIC;
        d->len          = l;
        d->sync_serial  = stripe->header->sync_serial;
        d->write_serial = stripe->header->write_serial;
      }
      ...
      io.aiocb.aio_fildes = fd;
      io.aiocb.aio_offset = stripe->header->write_pos;
  ...
  this->stripe_read_op<int>([&](const Stripe *stripe) {
    for (auto *c = static_cast<CacheVC *>(this->_write_buffer.get_pending_writers().head); c;) {
      ...
      if (this->_write_buffer.get_buffer_pos() + writelen > AGG_SIZE ||
          stripe->header->write_pos + this->_write_buffer.get_buffer_pos() + writelen > (stripe->skip + stripe->len)) {
        break;
      }
      DDbg(dbg_ctl_agg_read, "copying: %d, %" PRIu64 ", key: %d", this->_write_buffer.get_buffer_pos(),
           stripe->header->write_pos + this->_write_buffer.get_buffer_pos(), c->first_key.slice32(0));
...
StripeSM::_copy_evacuator_to_aggregation(CacheVC *vc)
{
  return this->stripe_read_op<int>([&](const Stripe *stripe) {
    ...
    int  approx_size = stripe->round_to_approx_size(doc->len);
    ...
    doc->write_serial = stripe->header->write_serial;

    off_t doc_offset{stripe->header->write_pos + this->_write_buffer.get_buffer_pos()};
    this->_write_buffer.add(doc, approx_size);

    vc->dir = vc->overwrite_dir;
    dir_set_offset(&vc->dir, stripe->offset_to_vol_offset(doc_offset));
...
{
  return this->stripe_read_op<int>([&](const Stripe *stripe) {
    off_t          doc_offset{stripe->header->write_pos + this->get_agg_buf_pos()};
    uint32_t       len         = vc->write_len + vc->header_len + vc->frag_len + sizeof(Doc);
    Doc           *doc         = this->_write_buffer.emplace(stripe->round_to_approx_size(len));
...
{
  this->stripe_write_op([&](Stripe *stripe) {
    stripe->header->write_pos = stripe->start;
    stripe->header->phase     = !stripe->header->phase;

    stripe->header->cycle++;
    stripe->header->agg_pos = stripe->header->write_pos;
    dir_lookaside_cleanup(this);
    dir_clean_vol(stripe);
    Metrics::Counter::increment(cache_rsb.directory_wrap);
    Metrics::Counter::increment(this->cache_vol->vol_rsb.directory_wrap);
    Note("Cache volume %d on disk '%s' wraps around", this->cache_vol->vol_number, this->hash_text.get());
    ink_assert(this->mutex->thread_holding == this_ethread());
    this->_preserved_dirs.periodic_scan(stripe);
  });
}
...
{
  return this->stripe_read_op<int>([&](const Stripe *stripe) {
    off_t s  = stripe->offset_to_vol_offset(low);
    off_t e  = stripe->offset_to_vol_offset(high);
    ...
    for (int i = si; i <= ei; i++) {
      ...
      if (first) {
        ...
        io.aiocb.aio_offset = stripe->vol_offset(&first->dir);
...
StripeSM::evacuateWrite(CacheEvacuateDocVC *evacuator, int event, Event *e)
{
  return this->stripe_read_op<int>([&](const Stripe *stripe) {
    ...
    evacuator->agg_len = stripe->round_to_approx_size((reinterpret_cast<Doc *>(evacuator->buf->data()))->len);
...
StripeSM::shutdown(EThread *shutdown_thread)
{
  ...
  this->stripe_write_op<int>([&](Stripe *stripe) {
    ...
    if (!stripe->header->dirty && !this->dir_sync_in_progress) {
    ...
    B            = pwrite(this->fd, stripe->raw_dir, dirlen, start);
    ink_assert(B == dirlen);
...
{
  return this->stripe_write_op<bool>([&](Stripe *stripe) {
    // set write limit
    stripe->header->agg_pos = stripe->header->write_pos + this->_write_buffer.get_buffer_pos();
