  ...
  do {
    ...
    if (!dir_valid(stripe, e) || !dir_offset(e)) {
      ...
      if (dir_offset(e)) {
        ...
        Metrics::decrement(stripe->cache_vol->vol_rsb.direntries_used);
      }
      ...
      if (dir_compare_tag(e, key)) {
        ...
        if (collision) {
          if (collision == e) {
            ...
            Metrics::increment(stripe->cache_vol->vol_rsb.directory_collision_count);
          }
          goto Lcont;
        }
        if (dir_valid(stripe, e)) {
...
Lfill:
  ...
  Metrics::increment(stripe->cache_vol->vol_rsb.direntries_used);

  return 1;
}

int
dir_overwrite(const CacheKey *key, Stripe *stripe, Dir *dir, Dir *overwrite, bool must_overwrite)
{
  ink_assert(stripe->mutex->thread_holding == this_ethread());
  int s          = key->slice32(0) % stripe->segments, l;
  int bi         = key->slice32(1) % stripe->buckets;
...
int
dir_lookaside_probe(const CacheKey *key, Stripe *stripe, Dir *result, EvacuationBlock **eblock)
{
  ink_assert(stripe->mutex->thread_holding == this_ethread());
...
sync_cache_dir_on_shutdown()
{
  ...
  for (int i = 0; i < gnstripe; i++) {
    ...
    MUTEX_TAKE_LOCK(gstripe[i]->mutex, t);
    Stripe *stripe = gstripe[i];
    ...
    if (stripe->agg_buf_pos) {
      Dbg(dbg_ctl_cache_dir_sync, "Dir %s: flushing agg buffer first", stripe->hash_text.get());

      // set write limit
      stripe->header->agg_pos = stripe->header->write_pos + stripe->agg_buf_pos;

      int r = pwrite(stripe->fd, stripe->agg_buffer, stripe->agg_buf_pos, stripe->header->write_pos);
      if (r != stripe->agg_buf_pos) {
        ink_assert(!"flushing agg buffer failed");
        continue;
      }
      stripe->header->last_write_pos  = stripe->header->write_pos;
      stripe->header->write_pos      += stripe->agg_buf_pos;
      ink_assert(stripe->header->write_pos == stripe->header->agg_pos);
      stripe->agg_buf_pos = 0;
...
Lrestart:
  ...
  Stripe *stripe = gstripe[stripe_idx]; // must be named "vol" to make STAT macros work.

  if (event == AIO_EVENT_DONE) {
    // AIO Thread
    if (!io.ok()) {
      Warning("vol write error during directory sync '%s'", gstripe[stripe_idx]->hash_text.get());
      event = EVENT_NONE;
      goto Ldone;
    }
    Metrics::increment(cache_rsb.directory_sync_bytes, io.aio_result);
    Metrics::increment(stripe->cache_vol->vol_rsb.directory_sync_bytes, io.aio_result);
    trigger = eventProcessor.schedule_in(this, SYNC_DELAY);
    return EVENT_CONT;
  }
  {
    CACHE_TRY_LOCK(lock, gstripe[stripe_idx]->mutex, mutex->thread_holding);
    if (!lock.is_locked()) {
    ...
    if (!writepos) {
      ...
      if (stripe->is_io_in_progress() || stripe->agg_buf_pos) {
    ...
    if (!writepos) {
      ...
      Metrics::increment(stripe->cache_vol->vol_rsb.directory_sync_count);
      Metrics::increment(cache_rsb.directory_sync_time, ink_get_hrtime() - start_time);
      Metrics::increment(stripe->cache_vol->vol_rsb.directory_sync_time, ink_get_hrtime() - start_time);
      start_time = 0;
      goto Ldone;
    }
    return EVENT_CONT;
