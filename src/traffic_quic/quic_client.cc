...
QUICClient::start(int, void *)
{
  ...
  SSL_CTX *ssl_ctx = quic_init_client_ssl_ctx(params); // group_list, session_file, keylog shoule be moved to here
  std::string_view alpn_list;
  if (this->_config->http3) {
    alpn_list = H3_ALPN_PROTO_LIST;
  } else {
    alpn_list = HQ_ALPN_PROTO_LIST;
  }

  if (SSL_CTX_set_alpn_protos(ssl_ctx, reinterpret_cast<const unsigned char *>(alpn_list.data()), alpn_list.size()) != 0) {
  ...
  for (struct addrinfo *info = this->_remote_addr_info; info != nullptr; info = info->ai_next) {
    ...
    Action *action = quic_NetProcessor.connect_re(this, info->ai_addr, &opt, ssl_ctx);
