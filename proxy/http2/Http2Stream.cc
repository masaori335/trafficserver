  ...
  case VC_EVENT_INACTIVITY_TIMEOUT:
    if (current_reader && read_vio.ntodo() > 0) {
      ...
      if (lock.is_locked()) {
        ...
        if (this->_read_vio_cont_event) {
          this->_read_vio_cont_event->cancel();
        }
        this->_read_vio_cont_event = this_ethread()->schedule_imm(read_vio.cont, event, &read_vio);
      }
      ...
      if (lock.is_locked()) {
        ...
        if (this->_write_vio_cont_event) {
          this->_write_vio_cont_event->cancel();
        }
        this->_write_vio_cont_event = this_ethread()->schedule_imm(write_vio.cont, event, &write_vio);
      }
    }
    break;
...
Http2Stream::clear_io_events()
{
  ...
  if (this->_read_vio_cont_event) {
    this->_read_vio_cont_event->cancel();
    this->_read_vio_cont_event = nullptr;
  }

  if (this->_write_vio_cont_event) {
    this->_write_vio_cont_event->cancel();
    this->_write_vio_cont_event = nullptr;
  }
}
