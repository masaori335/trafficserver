...
public:
  ...
  HPACK(uint32_t size, HPACK::Context c) : _dynamic_table(size, c) {}
  ~HPACK() {}
...
private:
  ...
  public:
    ...
    static HpackLookupResult lookup(const char *name, int name_len, const char *value, int value_len);

  private:
    static Index _lookup_name(const char *name, int name_len);
    static Index _lookup_value(Index begin, Index end, const char *value, int value_len);
  };

  class DynamicTable
  {
  public:
    explicit DynamicTable(uint32_t size, Context c);
    ~DynamicTable();
...
int64_t decode_indexed_header_field(MIMEFieldWrapper &header, const uint8_t *buf_start, const uint8_t *buf_end,
                                    HPACK &indexing_table);
int64_t decode_literal_header_field(MIMEFieldWrapper &header, const uint8_t *buf_start, const uint8_t *buf_end,
                                    HPACK &indexing_table);
int64_t update_dynamic_table_size(const uint8_t *buf_start, const uint8_t *buf_end, HPACK &indexing_table,
                                  uint32_t maximum_table_size);

// High level interfaces
using HpackHandle = HPACK;
